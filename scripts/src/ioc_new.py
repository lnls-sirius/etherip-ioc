...
def generate_conv_type_record(data: RowData, file_handler):
    generate_polyn_calc(
            file_handler, name=data.name, inp=data.inp,
            prec=data.prec, egu=data.egu, out=data.out,
            desc=data.desc, args=data.args)
    return
...
def generate_polyn_calc(f, name, inp, prec, egu, out="", desc="", args=""):
    f.write(
        acalcout_generic_template.safe_substitute(
            ...
            inpa=inp+" CP",
            inaa=args,
            calc="A*AA[1,1]^3+A*AA[2,2]^2+A*AA[3,3]+AA[4,4]",
            egu=egu,
            prec=prec,
            out=out,
            inpb="",
