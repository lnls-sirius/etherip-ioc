...
def generate_all(args):
    ...
    if args.multi_db is None and args.multi_cmd is None:
        generate_single(args)


def generate_single(args):
    generate(args, db=True, db_name=args.ioc_name, cmd=True, cmd_name=args.ioc_name)


def generate_multi_db(args):
    if args.multi_db is None:
        logger.error(
            "Multi DB option specified, but no DB name " "or prefix was provided."
        )
        return -1
    ...
    for prefix_and_name in args.multi_db:
        ...
        generate(args, db=True, db_name=db_name, cmd=False, new_prefix=prefix)
...
def generate(
    ...
    IOC_CMD_PATH = (
        os.path.join(BASE_PATH, "../ioc/iocBoot/iocetheripIOC/") + cmd_name + ".cmd"
    )

    IOC_DATABASE_PATH = os.path.join(BASE_PATH, "../ioc/database/") + db_name + ".db"
            ...
            for s_name in sheet_name:
                ...
                for n, row in sheet.iterrows():
                    ...
                    if not tag or type(tag) != str or tag == "" or tag == "N/A":
                        logger.warning(
                            "Tag not set! {}. EPICS record won't be generated.".format(
                                name
                            )
                        )
                        continue
                ...
                for tag, vals in tags.items():
                    if len(vals) > 1:
                        logger.warning(
                            'Tag "{}" already exists {}.'.format(tag, tags[tag])
                        )
